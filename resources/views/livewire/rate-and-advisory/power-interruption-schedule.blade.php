<div>
  ...
  <x-guest-section>
    ...
    @if (count($ongoingAdvisories) > 0)
      <div class="mb-8 scroll-reveal rounded-2xl border border-danger/20 bg-danger/4 overflow-hidden">
        <div class="flex items-center gap-3 px-5 py-3 bg-danger/8 border-b border-danger/15">
          ...
          <span class="text-xs font-bold uppercase tracking-wider text-danger">
            {{ count($ongoingAdvisories) }} Active Interruption{{ count($ongoingAdvisories) > 1 ? 's' : '' }} Right Now
          </span>
        </div>
        <div class="divide-y divide-danger/10">
          @foreach ($ongoingAdvisories as $a)
            <div class="flex items-start gap-4 px-5 py-4">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-tei-gray mt-0.5">{{ implode(', ', array_slice($a['areas'], 0, 3)) }}{{ \count($a['areas']) > 3 ? ' +' . (\count($a['areas']) - 3) . ' more' : '' }}</p>
              </div>
              <span class="text-xs font-bold text-danger shrink-0">{{ $a['time'] }}</span>
            </div>
          @endforeach
        </div>
    ...
    {{-- ── FILTER TABS ─────────────────────────────────────────── --}}
    <div class="scroll-reveal mb-6 flex flex-wrap gap-2">

      @php
        $tabs = [
          'all'       => ['label' => 'All',       'count' => $counts['all'],       'dot' => null],
          'scheduled' => ['label' => 'Scheduled', 'count' => $counts['scheduled'], 'dot' => 'bg-warning'],
          'ongoing'   => ['label' => 'Ongoing',   'count' => $counts['ongoing'],   'dot' => 'bg-danger'],
          'resolved'  => ['label' => 'Resolved',  'count' => $counts['resolved'],  'dot' => 'bg-success'],
        ];
      @endphp
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 stagger-cards">
        @foreach ($advisories as $advisory)
          @php
            ...
            $extraAreas = \count($advisory['areas']) - 3;
          @endphp
          ...
          <x-card :variant="$variant">
            ...
            <div class="flex items-center justify-between mb-3">
              ...
              {{-- Date --}}
              <span class="text-[11px] text-tei-gray-light font-medium">{{ $advisory['date'] }}</span>
            </div>
            ...
            <div class="flex items-center gap-1.5 mb-2">
              ...
              <span class="text-xs font-semibold {{ $timeColor }}">{{ $advisory['time'] }}</span>
            </div>
            ...
            <div class="flex items-start gap-1.5 mb-3">
              ...
              <span class="text-xs text-tei-gray leading-relaxed">
                {{ implode(', ', array_slice($advisory['areas'], 0, 3)) }}{{ $extraAreas > 0 ? ' +' . $extraAreas . ' more' : '' }}
              </span>
            </div>
            ...
            {{-- Footer --}}
            <x-slot:footer>
              <a href="#" class="inline-flex items-center gap-1.5 text-xs font-bold transition-colors duration-200
    ...
    {{-- ── DISCLAIMER ───────────────────────────────────────────── --}}
    <div class="mt-10 scroll-reveal rounded-2xl p-5 bg-tei-blue/3 border border-tei-blue/8">
