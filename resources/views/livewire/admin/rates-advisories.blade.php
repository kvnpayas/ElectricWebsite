      ...
      <x-table.body>
        @forelse ($this->documents as $doc)
          <tr class="transition-colors duration-150 hover:bg-gray-50
                     {{ $deleteTarget === $doc->id ? 'bg-danger/4' : '' }}">
            ...
            <x-table.td hidden="sm">
              @php
                ...
                $catColors = ['advisories' => 'bg-tei-blue/10 text-tei-blue', 'rate-schedule' => 'bg-tei-orange/10 text-tei-orange', 'others' => 'bg-gray-100 text-tei-gray'];
              @endphp
              <span class="inline-flex text-xs font-semibold px-2.5 py-1 rounded-full {{ $catColors[$doc->category] ?? 'bg-gray-100 text-tei-gray' }}">
            ...
            <x-table.td align="right">
              @if ($deleteTarget === $doc->id)
                <div class="flex items-center justify-end gap-2">
                  <span class="text-xs text-danger font-medium hidden sm:inline">Delete?</span>
                  <button wire:click="delete"
                          class="text-xs font-bold px-3 py-1.5 rounded-lg bg-danger text-white hover:bg-danger/80 transition-colors duration-150 cursor-pointer">
                    Yes
                  </button>
                  <button wire:click="cancelDelete"
                          class="text-xs font-bold px-3 py-1.5 rounded-lg bg-gray-100 text-tei-gray hover:bg-gray-200 transition-colors duration-150 cursor-pointer">
                    No
                  </button>
                </div>
              @else
    ...
    <form wire:submit.prevent="save" id="doc-form" class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
      ...
      <div>
        ...
        <input wire:model.live.debounce.400ms="formTitle" type="text"
               placeholder="Sample Document 2026"
