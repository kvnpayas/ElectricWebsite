...
{{-- Wrap in a plain div so the Livewire component root has no x-data conflict --}}
<div>
<header class="sticky top-0 z-30 flex h-16 shrink-0 items-center gap-3 border-b px-4 sm:px-6"
  ...
  <div class="flex items-center gap-1 sm:gap-2 shrink-0">
    ...
    {{-- ── Search palette (scoped x-data, no conflict with Livewire root) ── --}}
    <div
      x-data="{
        searchOpen: false,
        query: '',
        ...
        <div
          ...
          <div
            ...
            <div class="flex items-center gap-3 px-4 border-b" style="border-color: #F3F4F6;">
              ...
              <input x-ref="searchInput" x-model="query" type="text" placeholder="Search modules…"
                class="flex-1 py-4 text-sm bg-transparent outline-none placeholder-gray-400"
                style="color: #082840;" autocomplete="off" />
              <button @click="close()"
              ...
              <template x-if="filtered.length > 0">
                <ul>
                  <template x-for="(item, index) in filtered" :key="item.url">
                    <li>
                      <button @click="go(item.url)" @mouseenter="activeIndex = index"
    ...
    <div class="relative" x-data="{ userOpen: false }" @click.outside="userOpen = false">
      <button @click="userOpen = !userOpen"
        ...
        <svg class="size-4 transition-transform duration-200" :class="userOpen ? 'rotate-180' : ''" fill="none"
          stroke="currentColor" viewBox="0 0 24 24" style="color: #A7A8AA;">
      ...
      <div x-show="userOpen" x-cloak
        ...
        class="absolute right-0 top-full mt-2 w-52 rounded-2xl border shadow-xl overflow-hidden z-50"
        style="background-color: #FFFFFF; border-color: #E5E7EB;">
        ...
        <div class="border-t" style="border-color: #F3F4F6;">
          <button wire:click="logout"
            class="flex items-center gap-3 px-4 py-2.5 text-sm w-full text-left transition-colors duration-150 cursor-pointer"
