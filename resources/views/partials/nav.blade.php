...
<nav x-data="{
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between">
      ...
      <a href="/" class="flex flex-col leading-none group">
        ...
        <img src="{{ asset('assets/TEI-logo.png') }}" alt="Tarlac Electric Inc. logo" class="h-10 w-auto">
      </a>

      {{-- Desktop Nav --}}
      <div class="hidden lg:flex items-center gap-1">
        <a href="#" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer"
          style="color: rgba(255,255,255,0.9);" onmouseover="this.style.backgroundColor='rgba(255,255,255,0.1)'"
          onmouseout="this.style.backgroundColor='transparent'">Home</a>

        {{-- Services dropdown --}}
        <div class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
          <button
            class="flex items-center gap-1 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer"
            style="color: rgba(255,255,255,0.9);" onmouseover="this.style.backgroundColor='rgba(255,255,255,0.1)'"
          ...
          <div x-show="open" x-transition:enter="transition ease-out duration-150"
            ...
            @foreach ([['Bill Payment', 'Pay your electricity bill online'], ['New Connection', 'Apply for a new service connection'], ['Report Outage', 'Report power interruptions'], ['E-SOA Enrollment', 'Switch to paperless billing'], ['Senior Discount', 'Apply for senior citizen discount'], ['Power Rates', 'View current electricity rates']] as [$label, $desc])
              <a href="#"
                class="flex flex-col px-4 py-3 transition-colors duration-150 cursor-pointer border-b last:border-0"
      ...
      <div class="hidden lg:flex items-center gap-4">
        <a href="tel:+6345-606-1834" class="text-sm font-medium transition-colors duration-200"
          style="color: rgba(255,255,255,0.65);" onmouseover="this.style.color='white'"
  ...
  <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200"
    ...
    x-transition:leave-end="opacity-0 -translate-y-3" class="lg:hidden border-t mt-2 pb-4"
    ...
    <div class="max-w-7xl mx-auto px-4 pt-3 flex flex-col gap-1">
      @foreach (['Home', 'About', 'Advisories', 'Careers'] as $link)
        <a href="#" class="px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-150"
          style="color: rgba(255,255,255,0.85);" onmouseover="this.style.backgroundColor='rgba(255,255,255,0.08)'"
          onmouseout="this.style.backgroundColor='transparent'">{{ $link }}</a>
      @endforeach
      <div class="border-t my-2 pt-2" style="border-color: rgba(255,255,255,0.1);">
        <p class="px-4 py-2 text-xs font-bold uppercase tracking-widest" style="color: rgba(255,255,255,0.35);">Services
        </p>
        @foreach (['Bill Payment', 'New Connection', 'Report Outage', 'E-SOA Enrollment', 'Power Rates'] as $svc)
          <a href="#" class="px-4 py-2.5 text-sm font-medium rounded-lg block transition-colors duration-150"
            style="color: rgba(255,255,255,0.75);" onmouseover="this.style.backgroundColor='rgba(255,255,255,0.08)'"
