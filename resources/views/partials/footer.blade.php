@php
  ...
  $socialIcons = [
      'Facebook' => ['path' => 'M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z'],
      'YouTube' => [
          'path' =>
              'M22.54 6.42a2.78 2.78 0 00-1.94-1.96C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 00-1.94 1.96A29 29 0 001 11.75a29 29 0 00.46 5.33A2.78 2.78 0 003.4 19.1C5.12 19.56 12 19.56 12 19.56s6.88 0 8.6-.46a2.78 2.78 0 001.94-1.95 29 29 0 00.46-5.25 29 29 0 00-.46-5.33z M9.75 15.02l5.75-3.27-5.75-3.27v6.54z',
      ],
      'Instagram' => [
          'path' =>
              'M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37zm1.5-4.87h.01M6.5 6.5h11a.5.5 0 01.5.5v11a.5.5 0 01-.5.5h-11a.5.5 0 01-.5-.5v-11a.5.5 0 01.5-.5z',
      ],
      'Twitter' => [
          'path' =>
      ...
      'TikTok' => ['path' => 'M9 12a4 4 0 104 4V4a5 5 0 005 5'],
  ];
...
<footer class="bg-brand-header">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
        ...
        @if ($socials->isNotEmpty())
          <div class="flex items-center gap-3">
            @foreach ($socials as $social)
              @php $icon = $socialIcons[$social->platform] ?? ['path' => 'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14']; @endphp
      ...
      {{-- Customer column --}}
      <div>
        <h4 class="text-xs font-bold uppercase tracking-widest mb-5" style="color: rgba(255,255,255,0.35);">Customer</h4>
        <ul class="space-y-3">
          @foreach ([
            ['How to Read Your Bill',        'customer.how-to-read-your-bill'],
            ['Service Application',           'customer.service-application'],
            ['Bill Deposit',                  'customer.bill-deposit'],
            ['Senior Citizen Discount',       'customer.senior-citizen-discount'],
            ['Net Metering',                  'customer.net-metering-primer'],
            ['Distributed Energy Resources',  'customer.distributed-energy-resources'],
            ['Calculator',                    'customer.calculator'],
            ['Business Centers',              'customer.business-centers'],
          ] as [$item, $url])
            <li>
              <a href="{{ Route::has($url) ? route($url) : '#' }}" wire:navigate
                ...
                onmouseout="this.style.color='rgba(255,255,255,0.55)'">{{ $item }}</a>
            </li>
          @endforeach
        </ul>
      </div>
      ...
      <div>
        ...
        <ul class="space-y-3">
          @foreach ([
            ...
            ['Corporate Governance',  Route::has('about-us.corporate-governance') ? route('about-us.corporate-governance') : '#'],
            ['FAQs',                  Route::has('about-us.faqs') ? route('about-us.faqs') : '#'],
            ['Careers',               '#'],
        ...
        <div class="my-5 border-t" style="border-color: rgba(255,255,255,0.08);"></div>

        <h4 class="text-xs font-bold uppercase tracking-widest mb-5" style="color: rgba(255,255,255,0.35);">Rates & CSP</h4>
        <ul class="space-y-3">
          @foreach ([
            ['Rates & Advisories',          route('rate-and-advisories')],
            ['Power Interruption Schedule',  Route::has('rate-and-advisories.power-interruption-schedule') ? route('rate-and-advisories.power-interruption-schedule') : '#'],
            ['Power Supply Procurement',     Route::has('csp.power-supply-procurement') ? route('csp.power-supply-procurement') : '#'],
            ['Procurement Opportunities',    Route::has('procurement.opportunities') ? route('procurement.opportunities') : '#'],
          ] as [$item, $href])
            <li>
              <a href="{{ $href }}" wire:navigate
                class="text-sm transition-colors duration-150" style="color: rgba(255,255,255,0.55);"
        ...
        <ul class="space-y-4">
          ...
          @if ($address)
            <li class="flex items-start gap-3">
              ...
              <span class="text-sm" style="color: rgba(255,255,255,0.55);">{{ $address }}</span>
            </li>
          @endif
          ...
          @foreach ($phones as $phone)
            <li class="flex items-center gap-3">
              <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                style="color: var(--color-tei-orange);">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="{{ $phoneIcon[$phone->type] ?? $phoneIcon['landline'] }}" />
              </svg>
              <a href="{{ $phone->tel }}" class="text-sm transition-colors duration-150"
          ...
          @if ($primaryEmail)
            <li class="flex items-center gap-3">
              ...
              <a href="mailto:{{ $primaryEmail->address }}" class="text-sm transition-colors duration-150"
                style="color: rgba(255,255,255,0.55);" onmouseover="this.style.color='var(--color-tei-orange)'"
                onmouseout="this.style.color='rgba(255,255,255,0.55)'">{{ $primaryEmail->address }}</a>
            </li>
          @endif

          {{-- Emergency hotline (primary phone) --}}
          @if ($primaryPhone)
            <li>
              <div class="mt-1 p-3 rounded-xl" style="background-color: rgba(231,103,39,0.12);">
                ...
                <p class="text-base font-black" style="color: white;">{{ $primaryPhone->number }}</p>
